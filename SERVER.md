...
| 8079 | bugreport | systemd | `/var/www/html/bugreport` |
| 8090 | cardchaos | PM2 | `/var/www/html/cardchaos` |

Seit 08/2026 liegen alle Spiele im DocumentRoot und heissen wie ihr URL-Pfad.
...
# Nur bei Projekten mit WebSocket – Bugreport braucht diese zwei Zeilen nicht.
...
|---|---|
| Name | bugreport |
| Repo | https://github.com/gthdtasdnm/bugreport |
| Pfad auf dem Server | `/var/www/html/bugreport` |
| Port | 8079 |
| Prozessmanager | systemd (`bugreport.service`) |
...
**2. Der Dienst schreibt.** Deshalb `--allow-write` und die Datei außerhalb des
DocumentRoots – sonst wäre sie unter `https://inf-zeus.de/bugreport/data/…`
abrufbar (mit IP-Adressen darin) und ein `git pull` könnte sie überschreiben:

```bash
mkdir -p /var/lib/bugreport
chown www-data:www-data /var/lib/bugreport
```

Die Unit setzt dazu `Environment=BUGREPORT_DATA=/var/lib/bugreport/reports.json`.
`ProtectSystem=full` lässt `/var` beschreibbar, das passt zusammen.

**3. Ein Passwort für die Adminseite.** Vor dem ersten Start setzen, sonst
würfelt der Dienst bei jedem Neustart ein neues und niemand kommt mehr an die
Meldungen:

```bash
systemctl edit bugreport      # [Service] / Environment=BUGREPORT_ADMIN_TOKEN=…
systemctl restart bugreport
```

Das Token steht damit in `/etc/systemd/system/bugreport.service.d/override.conf`
und nicht im Repo. Prüfen, ohne es ins Log zu schreiben:
